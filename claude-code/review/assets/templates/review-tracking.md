...
- **Exam rounds**: The `/exam` command writes E columns (E1, E2, ...) interleaved chronologically with R columns. Each new column is APPENDED to the right of all existing round columns, so the header reads left-to-right in execution order (exam-led → E1 → R1 → E2; review-led → R1 → E1 → R2). Exam rounds use the same structural pattern as review rounds but with E prefix. The exam determines its round number by counting existing E columns only (not R columns). Review determines its round number by counting existing R columns only (not E columns).
- **[Item/Step/Task] placeholder**: Substitute based on document type: Design -> Item, Plan -> Step, Tasks -> Task.

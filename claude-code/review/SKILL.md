...
- `/review-triangulate <doc-path> [rounds] [notes]` -- multi-lane cross-validated deep review: runs the `review-loop` workflow (`workflows/review-loop.js`) while independent read-only subagents ground every claim against the repo (`file:line` evidence) and adversarially validate the chosen path (web-verified), then builds a convergence map (CONVERGED / WORKFLOW-ONLY / SUBAGENT-ONLY) and consolidates one verdict. The heavyweight variant of `/review-loop`; run in a top-level pane. Design/plan docs only

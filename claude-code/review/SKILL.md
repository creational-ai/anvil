...
- `/monitor <task-slug>` -- Monitor execution progress with periodic status reports; writes `docs/[slug]-monitor-issues.md` (lazy-created, appended across sessions) when issues are found (main conversation). **Agent-invocable** (`disable-model-invocation: false`) — a pane may run it as a watcher; it terminates on its own, either when every step completes or when the idle bound expires and it escalates

...
- **Optional**: Vision doc (`docs/[slug]-vision.md`) — provides problem statement, value proposition, and target user context
- **Optional**: Architecture doc (`docs/[slug]-architecture.md`) — provides technical surface area (CLI names, import names, API endpoints, MCP server names)
...
Read available design docs to understand:
...
For EVERY candidate, research these in order. Use WebSearch and WebFetch. Be thorough — a name that looks clean on first glance may have fatal conflicts buried in search results.
...
- WebSearch: `"[name]" company` and `"[name]" trademark`
- WebSearch: `"[name]" software` and `"[name]" [industry term]`
...
**d. SEO landscape:**
- WebSearch the bare name — what dominates results?
...
## Output
`docs/naming-research.md` using `assets/templates/naming-research.md`

Example: `docs/naming-research.md`

## Verification Checklist
- [ ] Template read from `assets/templates/naming-research.md`
- [ ] Output follows template structure exactly
- [ ] Vision doc read (if available) for context
- [ ] Architecture doc read (if available) for technical surfaces
...
- [ ] WebSearch used for trademark/company/namespace checks (not guessed)
...
## Next Stage
→ No fixed next stage. Naming research can be run at any point during design (typically after Vision or Architecture). The confirmed name feeds into all subsequent design docs.
